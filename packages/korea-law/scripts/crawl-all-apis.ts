...
interface ApiSpec {
  ...
  target: string;
  requestUrl: string;
  requestParams: { name: string; type: string; description: string }[];
  responseFields: { name: string; type: string; description: string }[];
  sampleUrls: { description: string; url: string }[];
  crawledAt: string;
}

const OUTPUT_DIR = path.join(__dirname, '../docs/api-specs/crawled');

async function extractApiSpec(page: Page, index: number): Promise<ApiSpec> {
  // @ts-ignore - evaluate 내부는 브라우저 컨텍스트에서 실행
  return await page.evaluate((idx: number) => {
    const h3s = document.querySelectorAll('h3');
    let apiName = '';
    h3s.forEach((h3: Element) => {
    ...
    let requestUrl = '';
    dts.forEach((dt: Element) => {
      if (dt.textContent?.includes('요청 URL')) {
        requestUrl = dt.textContent.replace('- 요청 URL :', '').trim();
      }
    });

    const tables = document.querySelectorAll('table');
    const requestParams: { name: string; type: string; description: string }[] = [];
    const responseFields: { name: string; type: string; description: string }[] = [];
    const sampleUrls: { description: string; url: string }[] = [];
...
async function crawlAllApis() {
  ...
  try {
    ...
    // 모든 API 링크 수집
    // @ts-ignore
    const apiLinks: { text: string }[] = await page.evaluate(() => {
      const links = document.querySelectorAll('table tbody a');
      return Array.from(links).map((link: Element) => ({
    ...
    for (let i = 0; i < apiLinks.length; i++) {
      ...
      try {
        ...
        errorCount++;
        console.error(`❌ [${i}] ${linkInfo.text} 크롤링 실패:`, err);
        allSpecs.push({
          index: i,
          name: linkInfo.text,
          target: '',
