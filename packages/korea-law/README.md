...
> (AI의 기억을 믿지 마세요. 현행법, 행정규칙, 판례로 감사(Audit)하세요.)
...
## 🎯 핵심 컨셉: "Legal Knowledge Graph"

이 MCP 서버는 단순한 법령 검색을 넘어, **법률-규칙-판례**가 연결된 지식 그래프를 지향합니다.

| 기능 | 설명 |
|------|------|
| **Statute Validity** | 인용된 법령이 현재 유효한가? (시행일 확인) |
| **Admin Rules** | 법률의 구체적 실행 지침(행정규칙)이 있는가? 🆕 |
| **Case Law** | 관련 대법원 판례나 헌재 결정이 있는가? 🆕 |
| **Interpretations** | 모호한 조항에 대한 정부 유권해석이 있는가? 🆕 |
...
> 📖 **상세 설정 가이드**: [MCP_SETUP_GUIDE.md](./MCP_SETUP_GUIDE.md)
>
> Claude Desktop, Cursor, Windsurf, Claude Code 등 다양한 AI 개발 도구에서 설정하는 방법을 확인하세요.
...
### 7. `get_related_laws` & `check_law_hierarchy`

법령의 위계(상위법/하위법)를 파악하고, 법령 간 충돌 시 우선순위를 판단합니다.

### 8. `search_legal_landscape` (종합 법령 검색) 🆕

**"법률(Code)만 보지 말고, 매뉴얼(Admin Rule)과 역사(Precedent)를 함께 보세요."**

법령, 행정규칙, 판례, 해석례를 한 번에 검색하여 입체적인 법적 근거를 제공합니다.

```
입력: { query: "CCTV 설치 기준" }
출력: 
  - 법률: 개인정보 보호법
  - 행정규칙: 영상정보처리기기 설치·운영 가이드라인 (중요!)
  - 해석례: 아파트 승강기 내부 CCTV 설치 관련 해석
```

### 9. `search_committee_decisions` (위원회 결정문) 🆕

**개인정보보호위원회, 공정거래위원회, 노동위원회 등 12개 규제기관 결정문 검색**

```
입력: { committee_type: "privacy", query: "개인정보 유출" }
출력: 개인정보보호위원회 결정례 목록 및 요지
```
...
### 10. `search_ministry_interpretations` (부처 해석례) 🆕

**고용노동부, 국세청, 국토부 등 13개 부처의 유권해석 검색**

```
입력: { ministry_type: "moel", query: "연차휴가" }
출력: 고용노동부 연차휴가 관련 질의회신 목록
```
...
### 11. `search_english_law` (영문 법령) 🆕

**한국 법령의 영문 번역본 검색 - 글로벌 비즈니스 지원**

```
입력: { query: "근로기준법", article_number: "제23조" }
출력: Labor Standards Act 영문 조문 + 한글 원문 대조
```

### 12. `search_tribunal_decisions` (특별행정심판) 🆕

**조세심판원, 특허심판원 등의 재결례 검색**

```
입력: { tribunal_type: "tax", query: "부당세금" }
출력: 조세심판원 재결례 목록
```

### 13. `search_compliance_all` (컴플라이언스 종합검색) 🆕

**법령 + 위원회 결정 + 부처 해석을 한 번에 검색하는 종합 도구**
