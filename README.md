...
## 설계 동기 — 왜 LLM 의존을 줄이고 법체계 구조를 살리는가

본 프로젝트의 주된 타깃 시나리오 중 하나는 **외부 API 호출이 제한되는 내부망(폐쇄망) 환경에서 로컬 LLM(Ollama 등) 기반 법률 자문·심사·감사 시스템을 구동**하는 것입니다. 공공기관·공무원·법무팀 등 민감 데이터를 외부 상용 LLM으로 내보낼 수 없는 환경에서, 소형 로컬 모델로도 의미 있는 품질의 법률 추론을 가능케 하려면 **모델 지능보다 데이터·인덱스 구조에서 얻는 레버리지**가 필요합니다.

### 성문법 국가의 이점을 구조로 인코딩

한국은 **성문법(civil law) 국가**로, 처분·판결은 원칙적으로 조문·판례·해석례의 위계 체계 위에서 도출됩니다. 사건 사실이 주어지면 적용 가능한 법조문은 어느 정도 **결정론적으로 좁혀질 수 있으며**, 판결 방향 역시 법체계 자체의 구조로 상당 부분 설명 가능합니다. LLM의 확률적 생성에만 기대어 발생하는 환각·일관성 문제는, 이 구조적 추론 여지를 충실히 인덱싱·노출하면 상당 부분 대체·보완됩니다.

본 리포의 모든 설계 결정은 이 전제 위에 있습니다.

| 설계 결정 | 이유 |
|---|---|
| 조문·별표 단위 indexing | LLM이 "법령 전체"가 아닌 **"제N조 제M항"** 수준에서 인용하게 — 판결문 스타일의 정확한 근거 제시 |
| BM25(nori + 법률 동의어 120+) + KNN(BGE-M3) + RRF | 벡터 유사성만으로는 조문 매칭 실패 — 법률 용어·동의어 기반 **문자열·술어 결정론**을 복원 |
| 쿼리 분류기 | 사건 사실을 "지자체명/법령명/처분 유형"으로 분해해 **적용 가능 조문 공간을 규칙 기반으로 우선 좁힘** (LLM 이전 단계) |
| `compare_ordinances_across_municipalities` | 동일 사실관계도 지자체 조례 차이로 결과가 갈리는 현실을 구조적 비교 도구로 노출 |
| 자치법규 우선 편입 | 국가법령만으로는 누락되는 **실제 처분 근거(지자체 조례·규칙)** 를 커버해 예측의 완결성 확보 |

### 궁극 목표

**사건 사실(factual pattern)만 입력해도 현행 법체계 하에서 도출 가능한 처분·판결 방향을 조문 단위 근거와 함께 설명**할 수 있는 수준. LLM은 최종 논리 합성기 역할을 수행하되, 조문 검색·우선순위·비교는 본 인덱스 구조가 담당합니다.

→ **내부망 Ollama 배포 환경에서도, 외부 상용 API 없이 법률 자문·심사·감사 업무를 의미 있는 품질로 수행**할 수 있게 하는 것이 본 리포의 궁극적 목표입니다.
...
| 호스팅 | 저자가 Fly.io에 공유 | self-host 가이드 + `law-check.com` Hosted Gateway |
